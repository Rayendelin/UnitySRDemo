...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        public static SRPlugin m_SRPlugin = new SRPlugin();
...
#endif
        {
...
#endif
            {
                ...
                // }
                {
                    camera.TryGetComponent<UniversalAdditionalCameraData>(out var baseAdditionalCameraData);
                    if (baseAdditionalCameraData != null && baseAdditionalCameraData.renderType != CameraRenderType.Base) {
                        Debug.LogError("Failed to AdditionalCameraData of the base camera.");
                        return;
                    }
                    var renderer = baseAdditionalCameraData?.scriptableRenderer;
                    
                    SRPlugin.SRMode mode = renderer.m_SRMode;
                    if (!m_SRPlugin.hasInit) {
                        m_SRPlugin.Init(renderContext, camera);
                    }

                    if (mode == SRPlugin.SRMode.CommonRender) {
                        renderContext.SetupCameraProperties(camera);
                        camera.targetTexture = m_SRPlugin.GetTargetTextureSR();
                        RenderCameraStack(renderContext, camera);
                        camera.targetTexture = null;
                        m_SRPlugin.PresentRenderFrameSR(renderContext, camera);
                    }
                    else if (mode == SRPlugin.SRMode.LowResolution) {
                        renderContext.SetupCameraProperties(camera);
                        camera.targetTexture = m_SRPlugin.GetTargetTextureLR();
                        RenderCameraStack(renderContext, camera);
                        camera.targetTexture = null;
                        m_SRPlugin.PresentRenderFrameLR(renderContext, camera);
                    }
                    else if (mode == SRPlugin.SRMode.SuperResolution) {
                        renderContext.SetupCameraProperties(camera);
                        camera.targetTexture = m_SRPlugin.GetTargetTextureLR();
                        RenderCameraStack(renderContext, camera);
                        camera.targetTexture = null;
                        m_SRPlugin.UpdateInputRT(renderContext, camera);
                        m_SRPlugin.RunSuperResolution(renderContext, camera);
                        m_SRPlugin.PresentResult(renderContext, camera);
                    }
                }
                else
                {
